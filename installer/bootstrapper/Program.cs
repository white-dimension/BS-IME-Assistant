...
internal static class Program
{
    ...
    private static int Install()
    {
        ...
        try
        {
            ...
            var cadBridgeSource = Path.Combine(payloadRoot, "cad", "BS-IME-Assistant-CadBridge.bundle");
            if (!Directory.Exists(appSource)) throw new DirectoryNotFoundException(appSource);
            if (!Directory.Exists(cadBridgeSource)) throw new DirectoryNotFoundException(cadBridgeSource);
            ...
            RemoveLegacyCadBundle();
            Remove3dsMaxStartup();
            CopyDirectory(cadBridgeSource, GetCadBridgeDir());
            Console.WriteLine($"Installed AutoCAD IME bridge: {GetCadBridgeDir()}");
    ...
    private static List<ExistingInstallation> DetectExistingInstallations()
    {
        ...
        {
            AddMatchingDirectories(results, "Legacy AutoCAD IME bundle", pluginRoot, "BS-CAD-Tools.bundle");
            AddMatchingDirectories(results, "AutoCAD IME bridge", pluginRoot, "BS-IME-Assistant-CadBridge.bundle");
        }
        ...
        if (Directory.Exists(maxUserRoot))
        {
            foreach (var versionDir in Directory.EnumerateDirectories(maxUserRoot))
            {
                ...
                AddPathIfExists(results, "Legacy 3ds Max IME bridge", Path.Combine(startupDir, "BS_IME_Startup.ms"));
                AddPathIfExists(results, "Legacy 3ds Max IME bridge", Path.Combine(startupDir, "BS_IME_Bridge.ms"));
            }
        }
    ...
    private static bool ConfirmOverwrite(IReadOnlyList<ExistingInstallation> existingInstallations)
    {
        ...
        {
            Console.WriteLine("No existing BS IME Assistant or legacy bridge files detected.");
            return true;
        }
        ...
        Console.WriteLine();
        Console.WriteLine("The app will be overwritten; legacy AutoCAD/3ds Max IME bridge files will be removed.");
    ...
    private static void RemoveLegacyCadBundle()
    {
        var cadBundle = GetLegacyCadBundleDir();
    ...
    private static void RemoveCadBridge()
    {
        var cadBridge = GetCadBridgeDir();
        if (Directory.Exists(cadBridge))
        {
            Directory.Delete(cadBridge, recursive: true);
            Console.WriteLine($"Removed AutoCAD IME bridge: {cadBridge}");
        }
    }
    ...
    private static string GetLegacyCadBundleDir()
    {
        return Path.Combine(RoamingAppData, "Autodesk", "ApplicationPlugins", "BS-CAD-Tools.bundle");
    }

    private static string GetCadBridgeDir()
    {
        return Path.Combine(RoamingAppData, "Autodesk", "ApplicationPlugins", "BS-IME-Assistant-CadBridge.bundle");
    }
